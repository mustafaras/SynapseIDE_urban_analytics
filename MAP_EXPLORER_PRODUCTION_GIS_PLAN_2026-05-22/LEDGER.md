...
- [x] 5 — Import preflight + profiling + support matrix ✅ verified (profiling/preflight + mandated CSV proof green; KML-polygon commit e2e wedge tracked as a render follow-up — see Done Log + Drift notes)
...
| --- | --- | --- | --- | --- |
| 2026-05-22 | 5 — Import preflight + profiling + support matrix | `gis/p05-profiling` | `105e417` (Prompts 3–5 committed together) | `SourceProfile` profiling added for ready imports, CSV row quality, large/empty/missing-CRS cases, external services, and profile-only partial formats; new preflight dialog gates ready imports before commit; CSV/columnar dialogs now show preflight facts; support matrix documented; `npm run typecheck` clean; `npm run lint:errors` clean; `npx vitest run src/services/map` 202 passed / 2 skipped (25 files); `npx vitest run src/centerpanel/components/map/__tests__/map-import-preflight.test.tsx` 2/2 passed; Playwright re-verified 2026-05-22: the prompt's mandated CSV proof (`3 skipped rows` + `CRS unknown`) and the GPX import test pass green; the KML-placemark test reproducibly times out at the commit click (headless **and** headed) — root-caused to committing a Polygon layer + animated `fitToBounds` wedging the MapLibre render pipeline, **not** a profiling-logic defect. Tracked as a Map Explorer render/commit follow-up (see Drift notes). Profiling unit coverage (skipped=3, missing/empty/large/external) all green. |
...
- Prompt 5 follow-up (2026-05-22 re-verification): re-running the e2e suite did **not** reproduce the previously-recorded "3/3". The mandated CSV proof and the GPX import test pass; the KML-placemark test reproducibly times out at the `Import Source` commit click in **both** headless and headed Chromium. Instrumented diagnosis: `node.click()` returns in ~3 ms, the preflight dialog DOM is stable (0 mutations) so it is not a render-loop in the dialog; the wedge starts **after commit** — committing the KML Polygon layer together with the animated `fitToBounds` (`duration: 900`, `easeTo`/`fitBounds`) leaves the MapLibre render pipeline non-idle so Playwright's `locator.evaluate` round-trip and teardown hang. `prefers-reduced-motion` (instant camera) gets past the click but the committed layer still does not surface within 20 s and `page.screenshot` itself stalls, so reduced-motion alone is not a complete fix. GPX (line/point geometry, no fill) commits fine through the identical path, isolating the trigger to **Polygon fill commit**. Also corrected in this pass: `profileCsvImportSession` reported `profileStrategy: "sampled"` while it counts every parsed session row — relabelled `"full-parse"` to match actual behaviour (no main-thread re-parse of the source). This KML render/commit wedge is a Map Explorer rendering concern (not Prompt 5 profiling logic) and is left as an explicit follow-up rather than papered over.
