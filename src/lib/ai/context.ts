...
import {
  EVIDENCE_SUMMARY_DEFAULT_LIMIT,
...
export function buildContextBundle(opts: {
  ...
  editor?: EditorBridge;
  /**
   * Prompt 25 — Evidence artifacts to surface as a compact metadata section
   * inside the context bundle. References only (ids, uris, confidence,
   * validation, caveats); never raw payloads. Bounded by `evidenceMaxChars`
   * (default 1600) and `evidenceLimit` (default 8).
   */
  evidence?: readonly SynapseArtifactEntry[];
  evidenceMaxChars?: number;
  evidenceLimit?: number;
}): BuiltContext {
  const { scope, tokenBudget, responseTokens, pinned = [], attachments = [], editor, evidence, evidenceMaxChars, evidenceLimit } = opts;
  ...
  const evidenceSection = evidence && evidence.length
    ? summarizeEvidenceForAi(evidence, {
        maxChars: evidenceMaxChars ?? EVIDENCE_SUMMARY_DEFAULT_MAX_CHARS,
        limit: evidenceLimit ?? EVIDENCE_SUMMARY_DEFAULT_LIMIT,
      })
    : '';
