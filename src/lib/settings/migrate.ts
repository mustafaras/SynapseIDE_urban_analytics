...
function toV2FromLegacy(s: AssistantSettings): SettingsStateV2 {
  ...
  const fallback = (listModelsByProvider((provider === 'auto' ? 'openai' : provider) as any)[0]?.id) || s.settings.model || PRIMARY_OPENAI_GPT5_MODEL;
  const model = provider === 'openai' && !isUsableOpenAIChatModel(s.settings.model || '')
    ? fallback
    : (s.settings.model || fallback);
