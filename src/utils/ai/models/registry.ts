...
import { inferOpenAIChatTokenCap, isUsableOpenAIChatModel, isUsableOpenAIGpt5Model, labelOpenAIChatModel } from './openaiGpt5';
...
export function getModel(id: string): ModelSpec | undefined {
  return MODEL_REGISTRY.find(m => m.id === id) || buildDynamicOpenAIChatSpec(id);
}

function buildDynamicOpenAIChatSpec(id: string): ModelSpec | undefined {
  if (!isUsableOpenAIChatModel(id)) {
    return undefined;
  }
  const isGpt5 = isUsableOpenAIGpt5Model(id);
  ...
  return {
    id,
    provider: 'openai',
    label: labelOpenAIChatModel(id),
    family,
    supportsTools: true,
    capTokens: inferOpenAIChatTokenCap(id),
