...
import {
  buildAutonomyDispatchPlan,
  summarizeAutonomyDispatchPlan,
  type AutonomyDispatchPlan,
} from './domainRouter';
...
export type OrchestratorResult = {
  ...
  planId?: string;
  decision?: string;
  dispatchPlan?: AutonomyDispatchPlan;
};
...
export async function runOrchestrator(cfg: OrchestratorConfig, input: OrchestratorInput): Promise<OrchestratorResult> {
  ...
  let round = 0;
  let lastCritique = '';
  const dispatchPlan = buildAutonomyDispatchPlan(input.userPrompt);
  const autonomyContext = summarizeAutonomyDispatchPlan(dispatchPlan);
  const plannerContext = [input.contextText, autonomyContext].filter(Boolean).join('\n\n');

  while (round < cfg.maxRounds && (performance.now() - tStart) < cfg.hardTimeMs) {
    round++;


    const planText = await withSpan('orchestrator.plan', { round }, async () => complete('Planner', input, SYSTEM_PLANNER, input.userPrompt, plannerContext));
  ...
  const plan: ActionsActionPlan = mapStructuredPlan(planParsed);
    ...
    if (ready) {
      return { status:'ok', rounds: round, planId: plan2.id, decision:'READY-TO-APPLY', dispatchPlan };
    }

    if ((performance.now() - tStart) > cfg.hardTimeMs) break;
  }

  return { status:'ok', rounds: round, decision: 'REQUIRES-MANUAL-REVIEW', dispatchPlan };
}
