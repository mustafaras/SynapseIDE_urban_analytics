...
/**
 * Legacy Editor Bridge  (compatibility layer)
 *
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  COMPATIBILITY LAYER — New callers should use the typed Synapse Bus     ║
 * ║  (`synapseBus`) from `@/services/synapseBus` instead.                  ║
 * ║                                                                          ║
 * ║  Existing callers (`editor:openTab`, `editor:insertAtCursor`,           ║
 * ║  `editor:replaceActive`, `editor:openRange`) continue to work unchanged.║
 * ║  The bridge adapter in `./bridgeAdapter.ts` forwards these events to    ║
 * ║  the typed Synapse Bus automatically.                                   ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 *
 * Payload contracts (documentation — do not change without updating bridgeAdapter.ts):
 *   editor:openTab          { filename: string; code: string; language?: string }
 *   editor:insertAtCursor   { code: string }
 *   editor:replaceActive    { code: string }
 *   editor:openRange        { path: string; fromLine: number; toLine: number }
 */
// Ensure the adapter is installed for this module's events to reach the typed bus.
import '@/services/editor/bridgeAdapter';
