...
export const createOpenAIAdapter = (apiKey: string): ProviderAdapter => ({
  async start({ requestId, signal, model, messages, temperature, topP, seed, onEvent }) {
    ...
    const useResponses = usesOpenAIResponses(model);
    const resp = await fetch(useResponses ? 'https://api.openai.com/v1/responses' : 'https://api.openai.com/v1/chat/completions', {
      method: 'POST',
