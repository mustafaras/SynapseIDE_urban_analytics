...
function generatePieChart(data: Array<{ type: string; value: number; percentage: number }>): string {
 ...
 REPORT_PALETTE.accentSecondary,
 REPORT_PALETTE.focus,
 '#B45309',
 '#92400E',
 '#78350F',
 ];
 ...
 <span style="color: ${REPORT_PALETTE.textPrimary};">${item.type}: ${item.percentage.toFixed(1)}%</span>
 </div>
 `).join('');
...
function generateLineChart(data: Array<{ date: string; duration: number; count: number }>): string {
 ...
 const circles = points.map(p => `<circle cx="${p.x}" cy="${p.y}" r="3" fill="${REPORT_PALETTE.accent}" />`).join('');


 const xLabels = points.map((p, i) =>
 i % 2 === 0 ? `<text x="${p.x}" y="${height - 5}" text-anchor="middle" font-size="9" fill="${REPORT_PALETTE.textMuted}">${p.date}</text>` : ''
 ).join('');
 ...
 return `<text x="${padding.left - 5}" y="${y}" text-anchor="end" font-size="9" fill="${REPORT_PALETTE.textMuted}">${Math.round(val)}m</text>`;
 }).join('');
 ...
 ${yTicks.map(val => {
 const y = padding.top + chartHeight - (val / maxDuration) * chartHeight;
 return `<line x1="${padding.left}" y1="${y}" x2="${width - padding.right}" y2="${y}" stroke="${REPORT_PALETTE.border}" stroke-width="1" />`;
 }).join('')}

 <!-- Line -->
 <path d="${pathData}" fill="none" stroke="${REPORT_PALETTE.accent}" stroke-width="2" />
...
function generateHeatmap(data: Array<{ date: string; intensity: number; count: number }>): string {
 ...
 const colors = [
 REPORT_PALETTE.heatLow,
 REPORT_PALETTE.heatMidLow,
 REPORT_PALETTE.heatMid,
 REPORT_PALETTE.heatMidHigh,
 REPORT_PALETTE.heatHigh,
 ];
 ...
 fill="${color}" rx="4" stroke="${REPORT_PALETTE.borderStrong}" stroke-width="1" />
 <text x="${x + cellSize/2}" y="${y + cellSize/2 + 5}"
 text-anchor="middle" font-size="12" fill="${REPORT_PALETTE.textPrimary}" font-weight="600">${dateLabel}</text>
 ${d.count > 0 ? `<text x="${x + cellSize - 4}" y="${y + 10}" text-anchor="end" font-size="8" fill="${REPORT_PALETTE.textPrimary}" opacity="0.75">${d.count}</text>` : ''}
 </g>
 `;
 }).join('');
 ...
 text-anchor="middle" font-size="11" fill="${REPORT_PALETTE.textMuted}" font-weight="600">${day}</text>
 `).join('');
 ...
 <div style="display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 16px; font-size: 11px; color: ${REPORT_PALETTE.textMuted};">
 <span>Less</span>
...
function generateBarChart(data: Array<{ hour: number; count: number }>): string {
 ...
 return `
 <rect x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" fill="${REPORT_PALETTE.accentSecondary}" />
 <text x="${x + barWidth / 2}" y="${height - 5}" text-anchor="middle" font-size="9" fill="${REPORT_PALETTE.textMuted}">${d.hour}h</text>
 `;
 }).join('');
 ...
 return `
 <line x1="${padding.left}" y1="${y}" x2="${width - padding.right}" y2="${y}" stroke="${REPORT_PALETTE.border}" stroke-width="1" />
 <text x="${padding.left - 5}" y="${y}" text-anchor="end" font-size="9" fill="${REPORT_PALETTE.textMuted}">${Math.round(val)}</text>
 `;
 }).join('');
...
function generateReportHTML(options: ExportOptions): string {
 const { sessions, chartData, organizationName = 'Urban Analytics Lab', analystName = 'Analyst', reportTitle = 'Spatial Analytics Report' } = options;
 ...
 .map(([type, stats]) => `
 <tr>
 <td style="padding: 8px 12px; border-bottom: 1px solid ${REPORT_PALETTE.border};">${type}</td>
 <td style="padding: 8px 12px; text-align: center; border-bottom: 1px solid ${REPORT_PALETTE.border};">${stats.count}</td>
 <td style="padding: 8px 12px; text-align: right; border-bottom: 1px solid ${REPORT_PALETTE.border};">${Math.round(stats.duration / 60000)} min</td>
 <td style="padding: 8px 12px; text-align: right; border-bottom: 1px solid ${REPORT_PALETTE.border};">${Math.round((stats.duration / totalDuration) * 100)}%</td>
 </tr>
 `)
 .join('');
 ...
 .map((session) => `
 <tr>
 <td style="padding: 8px 12px; border-bottom: 1px solid ${REPORT_PALETTE.border}; font-size: 12px;">
 ${format(new Date(session.startTime), 'MMM dd, yyyy HH:mm')}
 </td>
 <td style="padding: 8px 12px; text-align: center; border-bottom: 1px solid ${REPORT_PALETTE.border};">
 ${Math.round(session.totalDuration / 60000)} min
 </td>
 <td style="padding: 8px 12px; text-align: center; border-bottom: 1px solid ${REPORT_PALETTE.border};">
 ${session.laps?.length || 0}
 </td>
 <td style="padding: 8px 12px; border-bottom: 1px solid ${REPORT_PALETTE.border}; font-size: 11px;">
 ...
 <div class="metric-value" style="color: ${REPORT_PALETTE.accent};">${chartData.metrics.seq}%</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon"></div>
 <div class="metric-label">CONSISTENCY LOAD</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.focus};">${chartData.metrics.consistencyLoad}/10</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon"></div>
 <div class="metric-label">TOTAL SESSIONS</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.accentSecondary};">${chartData.metrics.totalSessions}</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon">⏱</div>
 <div class="metric-label">AVG DURATION</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.accent};">${chartData.metrics.avgDuration.toFixed(1)}</div>
 <div class="metric-unit">min</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon"></div>
 <div class="metric-label">TOTAL TIME</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.accentSecondary};">${chartData.metrics.totalTime.toFixed(1)}</div>
 <div class="metric-unit">hr</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon">✓</div>
 <div class="metric-label">AVG CHECKPOINTS</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.focus};">${chartData.metrics.avgCheckpoints.toFixed(1)}</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon"></div>
 <div class="metric-label">7-DAY AVG</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.accent};">${chartData.metrics.seqAvg.toFixed(1)}</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon"></div>
 <div class="metric-label">COMPLETION</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.focus};">${chartData.metrics.completion}%</div>
 </div>
 <div class="enhanced-metric-card">
 <div class="metric-icon"></div>
 <div class="metric-label">VARIABILITY</div>
 <div class="metric-value" style="color: ${REPORT_PALETTE.textMuted};">±${chartData.metrics.variability}</div>
 <div class="metric-unit">min</div>
 </div>
 </div>
 </div>
 ...
 <div class="insight-value" style="color: ${REPORT_PALETTE.focus}; font-size: 48px; font-weight: 700;">${chartData.insights.consistencyScore}%</div>
 ...
 * {
 ...
 color: ${REPORT_PALETTE.textPrimary};
 line-height: 1.6;
 padding: 40px;
 background: ${REPORT_PALETTE.base};
 }
 .header {
 border-bottom: 3px solid ${REPORT_PALETTE.accent};
 padding-bottom: 20px;
 margin-bottom: 30px;
 }
 .header h1 {
 ...
 .header .clinic-name {
 font-size: 16px;
 color: ${REPORT_PALETTE.accent};
 font-weight: 600;
 margin-bottom: 4px;
 }
 .header .meta {
 font-size: 12px;
 color: ${REPORT_PALETTE.textMuted};
 }
 .section {
 margin-bottom: 32px;
 }
 ...
 margin-bottom: 16px;
 padding-bottom: 8px;
 border-bottom: 2px solid ${REPORT_PALETTE.border};
 }
 .metrics-grid {
 display: grid;
 ...
 .metric-card {
 background: ${REPORT_PALETTE.surface1};
 ...
 background: ${REPORT_PALETTE.surface2};
 border: 1px solid ${REPORT_PALETTE.border};
 border-radius: 8px;
 overflow: hidden;
 }
 ...
 margin-top: 12px;
 padding-top: 12px;
 border-top: 1px solid ${REPORT_PALETTE.border};
 }
 .heatmap-container {
 background: ${REPORT_PALETTE.surface1};
 border: 1px solid ${REPORT_PALETTE.border};
 ...
 .insight-card {
 background: linear-gradient(135deg, ${REPORT_PALETTE.surface2} 0%, ${REPORT_PALETTE.surface1} 100%);
 border: 2px solid ${REPORT_PALETTE.accent};
 border-radius: 8px;
 padding: 24px;
 text-align: center;
 }
 ...
 align-items: center;
 padding: 10px 0;
 border-bottom: 1px solid ${REPORT_PALETTE.border};
 }
...
<body>
 ...
 This report contains project data. Handle in accordance with data privacy standards.
 </div>
 </div>
</body>
</html>
...
export async function exportAnalyticsToPDF(options: ExportOptions): Promise<void> {
 ...
 filename: `spatial-analytics-${format(new Date(), 'yyyy-MM-dd')}.pdf`,
 ...
 element.style.background = REPORT_PALETTE.base;
