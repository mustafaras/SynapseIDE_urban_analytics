...
type EnvValue = boolean | string | undefined;
type ThemeMode = 'dark' | 'neutral';
type CanaryEnvironment = 'prod' | 'staging';
type WindowFlags = {
  consultonCanary?: boolean;
  synapseCoreAI?: boolean;
};

function envRecord(): Record<string, EnvValue> {
  return (import.meta as ImportMeta & { env?: Record<string, EnvValue> }).env ?? {};
}

function env(key: string, fallback?: EnvValue): EnvValue {
  return envRecord()[key] ?? fallback;
}

function windowFlags(): WindowFlags | undefined {
  if (typeof window === 'undefined') {
    return undefined;
  }

  return (window as Window & { flags?: WindowFlags }).flags;
}
...
const CANARY_ENV: CanaryEnvironment = env('VITE_CONSULTON_CANARY_ENV', 'staging') === 'prod'
  ? 'prod'
  : 'staging';
...
export const flags = {
  ...
  themeMode: (() => {
    try {
      if (typeof window !== 'undefined') {
        ...
        if (qs === 'dark' || qs === 'neutral') return qs;
      }
      if (typeof localStorage !== 'undefined') {
        const v = localStorage.getItem('synapse.theme.mode');
        if (v === 'dark' || v === 'neutral') return v;
      }
      const envMode = env('VITE_THEME_MODE');
      if (envMode === 'dark' || envMode === 'neutral') return envMode;
    } catch {}
    return 'dark';
  })() as ThemeMode,
  ...
  simpleStream: (() => {
    try {
      ...
      const envValue = env('VITE_SIMPLE_STREAM');
      if (envValue === '1' || envValue === 'true') return true;
    } catch {}
    return true;
  })(),
  ...
  synapseCoreAI: (() => {
    try {
      if (typeof window !== 'undefined') {
        const flagsFromWindow = windowFlags();
        if (typeof flagsFromWindow?.synapseCoreAI === 'boolean') {
          return flagsFromWindow.synapseCoreAI;
        }
      ...
      const envValue = env('VITE_SYN_CORE_AI');
      if (envValue === '1' || envValue === 'true') return true;
    } catch {}
    return true;
  })(),
  ...
  consultonAI: (() => {
    try {
      ...
      const v1 = env('VITE_CONSULTON_AI');
      const v2 = env('VITE_FEATURE_CONSULTON_AI');
      const val = v1 ?? v2;
...
export function getConsultonCanaryOverride(): CanaryOverride {
  try {
    if (typeof window !== 'undefined') {
      const flagsFromWindow = windowFlags();
      if (typeof flagsFromWindow?.consultonCanary === 'boolean') {
        return flagsFromWindow.consultonCanary ? 'on' : 'off';
      }
