...
export interface ContextPrefs {
  ...
  /** Include active diagnostics (errors/warnings) in AI context. Off by default. */
...
function markAiSettingsHydrated() {
  try {
    if (!useAiSettingsStore.getState().hydrated) {
      useAiSettingsStore.setState({ hydrated: true });
    }
    window.dispatchEvent(new CustomEvent('ai:settingsHydrated'));
  } catch {}
}
...
const initial: Omit<AiSettingsState, 'setKey' | 'clearKey' | 'clearAllKeys' | 'setDefaults' | 'setUI' | 'setContext' | 'setTokenBudget' | 'setAdvanced' | 'setFlags' | 'getDecryptedKey'> = {
  ...
  defaults: { provider: 'openai', model: 'gpt-5-mini', temperature: 0.2, maxTokens: 2048 },
  ui: { panelWidth: 450, fontSize: 13, compactMode: false, autoInsertToEditor: false, confirmOverwrite: true },
  context: { scope: 'selection', includeComments: false, includeDiagnostics: false },
  tokenBudget: 8000,
...
try {
  ...
  if (useAiSettingsStore.persist.hasHydrated()) {
    markAiSettingsHydrated();
  }
} catch {}
