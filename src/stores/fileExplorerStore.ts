              ...
              features: [
                {
                  ...
                  properties: {
                    ...
                    visitors_per_year_million: 13.6,
                  },
                  geometry: { type: 'Point', coordinates: [28.97993, 41.00858] },
                },
                {
                  type: 'Feature',
                  id: 'landmark-sultanahmet',
                  properties: {
                    name: 'Sultanahmet Camii',
                    kind: 'landmark',
                    district: 'Fatih',
                    category: 'historic-religious',
                    visitors_per_year_million: 9.2,
                  },
                  geometry: { type: 'Point', coordinates: [28.97682, 41.00541] },
                },
                {
                  type: 'Feature',
                  id: 'landmark-topkapi',
                  properties: {
                    ...
                    visitors_per_year_million: 4.7,
                  },
                  geometry: { type: 'Point', coordinates: [28.98329, 41.01152] },
                },
                {
                  type: 'Feature',
                  id: 'landmark-yerebatan',
                  properties: {
                    name: 'Yerebatan Sarnici',
                    kind: 'landmark',
                    district: 'Fatih',
                    category: 'historic-cistern',
                    visitors_per_year_million: 3.1,
                  },
                  geometry: { type: 'Point', coordinates: [28.97799, 41.00831] },
                },
                {
                  type: 'Feature',
                  id: 'zone-sultanahmet-square',
                  properties: {
                    name: 'Sultanahmet Meydani Etki Alani',
                    kind: 'analysis-zone',
                    district: 'Fatih',
                    category: 'pedestrian-core',
                  },
                  geometry: {
                    type: 'Polygon',
                    coordinates: [[[28.9755, 41.0105], [28.9828, 41.0105], [28.9828, 41.0042], [28.9755, 41.0042], [28.9755, 41.0105]]],
                  },
                },
              ],
            }),
...
export const MAX_FILE_SEARCH_QUERY_CHARS = 200;
...
const clampFileTree = (files: FileNode[], maxNodes = MAX_PERSISTED_TREE_NODES): FileNode[] => {
  let seen = 0;
...
const prepareFilesForPersistence = (files: FileNode[]): FileNode[] => {
  const trimmed = clampFileTree(files);
  const stripLargeContent = (nodes: FileNode[]): FileNode[] =>
    nodes.map(node => {
      const next: FileNode = { ...node };
      if (
        next.type === 'file' &&
        typeof next.content === 'string' &&
        next.content.length > MAX_PERSISTED_FILE_CONTENT_CHARS
      ) {
        delete next.content;
      }
      if (next.children?.length) {
        next.children = stripLargeContent(next.children);
      }
      return next;
    });
  return stripLargeContent(trimmed);
};
  ...
  persist(
    immer((set, get) => ({
      ...
      setSearchQuery: query =>
        set(state => {
          state.searchQuery = query.slice(0, MAX_FILE_SEARCH_QUERY_CHARS);
        }),
    ...
    })),
    {
      ...
      storage: {
        getItem: name => {
          ...
          if (Array.isArray(persisted.state?.expandedFolders)) {
            persisted.state.expandedFolders = persisted.state.expandedFolders
        ...
        setItem: (name, value) => {
          try {
            ...
            // Persistence is best-effort for the in-memory file tree. Large files
            // remain available in-session but are not forced into localStorage.
          }
        },
        removeItem: name => {
