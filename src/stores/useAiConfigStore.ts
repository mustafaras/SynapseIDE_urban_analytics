...
const createSlice = (set: SetFn, get: () => (AiConfigState & Internal)): (AiConfigState & Internal) => ({
  ...
  async refreshModels(p?: ProviderId) {
    ...
    const merged = provider === 'openai' && remote.length
      ? remote
      : Array.from(new Set([...staticList, ...remote]));
    set((s: AiConfigState & Internal) => ({
      modelList: { ...s.modelList, [provider]: merged },
      model: resolveModel(s.model, merged, provider),
      modelListFetchedAt: { ...s.modelListFetchedAt, [provider]: remote.length ? Date.now() : s.modelListFetchedAt?.[provider] },
    }));
  },
...
export const useAiConfigStore = create<AiConfigState & Internal>()(persist(
  ...
  const draft = prev as UnknownRecord;
    ...
    onRehydrateStorage: () => () => {
      // Auto-refresh stale model lists on startup
      setTimeout(() => {
        try {
          const st = _lastGet();
          ...
          for (const p of providers) {
            ...
            if (!isStale) continue;
            setTimeout(() => { void _lastGet().refreshModels(p); }, delay);
            delay += 800; // stagger requests
          }
        } catch { /* silent */ }
      }, 1500); // wait for store to fully settle
  ...
  const updated: Record<ProviderId, string[]> = { ...st.modelList } as Record<ProviderId, string[]>;
        ...
        for (const p of providers) {
          ...
          const merged = p === 'openai'
            ? [...seeds]
            : Array.from(new Set([...seeds, ...existing]));
          if (!sameModelList(merged, existing)) { updated[p] = merged; mutated = true; }
        }
        const nextModel = resolveModel(st.model, updated[st.provider] || getStaticModels(st.provider), st.provider);
        if (mutated || st.registryVersion !== __MODEL_REGISTRY_VERSION || nextModel !== st.model) {
          _lastSet({ modelList: updated, model: nextModel, registryVersion: __MODEL_REGISTRY_VERSION });
        }
      } catch {}
    }
  }
...
function sameModelList(left: ModelId[], right: ModelId[]) {
