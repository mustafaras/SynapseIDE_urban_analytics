...
export function buildHTML(note: NoteSlots): string {
  ...
  const css = `
  <style>
    body{font-family:var(--font-mono, ui-monospace, SFMono-Regular, Menlo, Consolas, "Liberation Mono", monospace); color:#FAFAF9; background:#000000; padding:24px;}
    h1,h2{margin:0 0 8px 0}
    h1{font-size:20px} h2{font-size:16px}
    p{margin:0 0 10px 0; line-height:1.5}
    @media print { body{background:#fff; color:#000} }
  </style>`;
  return `<!doctype html><meta charset="utf-8"><title>Session Note</title>${css}<p>${md}</p>`;
}
...
export function buildRegistryJSON(
  ...
  const payload = opts.anonymize ? anonymizeProjects(projects) : projects;
  return JSON.stringify({ kind: "registry.v1", projects: payload }, null, 2);
}


export function parseRegistryJSON(json: string): Patient[] {
  const parsed = JSON.parse(json);
  if (parsed && parsed.kind === "registry.v1" && Array.isArray(parsed.projects ?? parsed.patients)) {
    return (parsed.projects ?? parsed.patients) as Patient[];
  }
  if (Array.isArray(parsed)) return parsed as Patient[];
  throw new Error("Unrecognized JSON: expected { kind:'registry.v1', projects: [...] }");
}


export function buildRegistryCSV(projects: Patient[]): string {
  const header = [
    "id",
    "name",
    "age",
  ...
  const rows = projects.map((p) => {
    const encounters = (p.sessions ?? p.encounters ?? []) as Array<{ when: number }>;
    const lastEnc = encounters.slice().sort((a, b) => b.when - a.when)[0];
    const openTasks = (p.tasks ?? []).filter(t=>!t.done).length;
    const kinds = [...new Set((p.assessments ?? []).map(a => a.kind))].sort();
    const [i1L, i1P] = kinds[0] ? lastTwoScores(p, kinds[0] as any) : [undefined, undefined];
    const [i2L, i2P] = kinds[1] ? lastTwoScores(p, kinds[1] as any) : [undefined, undefined];
    const [i3L, i3P] = kinds[2] ? lastTwoScores(p, kinds[2] as any) : [undefined, undefined];
    return [
      csv(p.id),
      csv(p.name ?? ""),
      csv(p.age ?? ""),
...
function anonymizeProjects(projects: Patient[]): Patient[] {
...
function _makeSamplePreview(input: { projectId?: string|number; name?: string; age?: number; sex?: string; note?: string; address?: any; lastSeen?: string | number | Date | null }, policy: DeidPolicy): DeidNotes {
  ...
  }, policy, (input.projectId == null ? undefined : { projectId: input.projectId }));
  const lines = [
...
export function deidApplyRecord<T extends Record<string, any>>(rec: T, policy: DeidPolicy, context?: { projectId?: string|number }): T {

  const out: any = Array.isArray(rec) ? [...(rec as any)] : { ...(rec as any) };
  const pid = String(context?.projectId ?? (rec as any).projectId ?? (rec as any).id ?? '');


  if (policy.preset !== 'none') {
    const salt = policy.saltRef;
    if (out.projectId != null) out.projectId = _fnv1aSalted(String(out.projectId), salt);
    if (out.sessionId != null) out.sessionId = _fnv1aSalted(String(out.sessionId), salt);
    if (out.id != null && out.id !== out.projectId) out.id = _fnv1aSalted(String(out.id), salt);
  }
...
export function previewAsPdfHtml(columns: PreviewColumn[], rows: PreviewRow[]): string {
  const css = `
    <style>
      ...
      .card{border:1px solid rgba(255,255,255,0.12); border-radius:12px; padding:12px; margin:8px 0; background:#1A1A1A;}
