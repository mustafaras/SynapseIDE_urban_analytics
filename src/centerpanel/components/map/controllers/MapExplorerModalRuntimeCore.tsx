...
function buildTopbarViewControlsButtonStyle(active: boolean): React.CSSProperties {
  return {
    display: 'inline-flex',
    alignItems: 'center',
    ...
    border: active
      ? `1px solid color-mix(in srgb, ${MAP_COLORS.interaction} 50%, transparent)`
      : MAP_STROKES.hairlineSubtle,
    borderRadius: 0,
    background: active
      ? `color-mix(in srgb, ${MAP_COLORS.interaction} 14%, transparent)`
      : 'color-mix(in srgb, var(--syn-surface-subtle, rgba(15, 23, 42, 0.44)) 70%, transparent)',
...
const drawModalBodyStyle: React.CSSProperties = {
  display: "flex",
...
const drawModalFooterStyle: React.CSSProperties = {
  justifyContent: "space-between",
  minHeight: "2.25rem",
  padding: `${MAP_SPACING.xs} ${MAP_SPACING.md}`,
};

const drawModalFooterMetaStyle: React.CSSProperties = {
  minWidth: 0,
  overflow: "hidden",
  textOverflow: "ellipsis",
  whiteSpace: "nowrap",
...
function getDefaultCompareBaseLayer(activeBaseLayer: BaseLayerId): BaseLayerId {
  return activeBaseLayer === 'satellite' ? 'streets' : 'satellite';
}
...
export const MapExplorerModal: React.FC<MapExplorerModalProps> = ({ open, onClose, mode = 'modal', mapCanvasRef, bottomTimelineSlot }) => {
  ...
  const [showSwipeCompare, setShowSwipeCompare] = useState(false);
  ...
  const handleCollapseRightDockHost = useCallback(() => {
    setRightDockCollapsed(current => {
      const next = !current;
      announce(next ? 'Right dock collapsed to rail' : 'Right dock expanded');
      return next;
    });
  }, [announce]);
  ...
  const handleAddDrawingsAsLayer = useCallback(() => {
    ...
    const layerId = `drawings-${Date.now().toString(36)}`;
    addOverlayLayer({
      id: layerId,
      name: `Drawings (${drawnFeatures.length} feature${drawnFeatures.length === 1 ? '' : 's'})`,
      type: 'geojson',
      visible: true,
      opacity: 0.9,
      sourceData: geojson,
  ...
  useEffect(() => {
    if (swipeCompareBaseLayer === activeBaseLayer) {
      setSwipeCompareBaseLayer(getDefaultCompareBaseLayer(activeBaseLayer));
    }
  }, [activeBaseLayer, swipeCompareBaseLayer]);

  const handleToggleCanvasSwipeCompare = useCallback(() => {
    setShowSwipeCompare(current => {
      const next = !current;
      if (next && swipeCompareBaseLayer === activeBaseLayer) {
        setSwipeCompareBaseLayer(getDefaultCompareBaseLayer(activeBaseLayer));
      }
      announce(next ? 'Swipe compare enabled' : 'Swipe compare disabled');
      return next;
    });
  }, [activeBaseLayer, announce, swipeCompareBaseLayer]);
    ...
    <MapWorkspaceShell mode={mode} shellRef={trapRef} onClose={handleMapExplorerCloseRequest} activeActivityId={activeActivityId}>
      ...
      <MapTopCommandSurface
        ...
        mapToolsSlot={
          <button
            type="button"
            style={buildTopbarViewControlsButtonStyle(showCanvasControlDock)}
            aria-label={showCanvasControlDock ? 'Hide draggable view controls' : 'Show draggable view controls'}
            aria-pressed={showCanvasControlDock}
            aria-expanded={showCanvasControlDock}
            aria-controls="map-canvas-control-dock"
            data-testid="map-view-controls-trigger"
            title={showCanvasControlDock ? 'Hide draggable view controls' : 'Show draggable view controls'}
            onClick={() => {
              setShowCanvasControlDock(current => {
                const next = !current;
                announce(next ? 'View controls opened' : 'View controls closed');
                return next;
              });
            }}
          >
            <SlidersHorizontal size={MAP_ICON_SIZES.xs} strokeWidth={1.8} aria-hidden="true" />
            <span>Controls</span>
          </button>
        }
        ...
        style={
          {
            ...
            '--map-canvas-control-dock-width': '58rem',
      ...
      >
        ...
        {!navigatorStageMode ? (
          <MapSwipeCompareOverlay
            visible={showSwipeCompare}
            baseLayer={activeBaseLayer}
            compareBaseLayer={swipeCompareBaseLayer}
            overlayLayers={mapRenderLayers}
            pins={pins}
            viewport={{ center, zoom, bearing, pitch }}
            reducedMotion={reducedMotion}
            onCompareBaseLayerChange={setSwipeCompareBaseLayer}
            onClose={() => {
              setShowSwipeCompare(false);
              announce('Swipe compare disabled');
            }}
            onAnnounce={announce}
          />
        ) : null}
          ...
          <MapDialogShell
            ...
            footer={(
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', minWidth: 0, gap: '8px' }}>
                ...
                <span style={drawModalFooterMetaStyle}>
                  {drawnFeatures.length.toLocaleString()} {drawnFeatures.length === 1 ? 'drawing' : 'drawings'} / {activeDrawTool ?? 'select'}
                </span>
              </div>
            )}
            footerStyle={drawModalFooterStyle}
            onClose={() => {
              setShowDrawPanel(false);
              announce('Drawings modal closed');
            }}
