...
import { buildMapPremiumMenuModel, MapPremiumMenuBar } from "./shell";
...
import { AppDropdownMenu, AppMenuItem, AppMenuSection, AppPopover, ToolbarMenuButton } from "./ui";
...
export type ToolbarDensity = MapToolbarDensityPreference;
...
type ToolbarMenuSectionId = "workspace" | "view" | "tools" | "settings";
...
const COMMAND_CENTER_OVERFLOW_COMMAND_IDS = [
  "undo-map-action",
  "redo-map-action",
  "reset-layout",
  "collapse-panels",
...
const roleSwitch: React.CSSProperties = {
  ...
  height: "1.75rem",
  padding: "0 0.125rem",
  border: "1px solid color-mix(in srgb, var(--syn-border-subtle, rgba(148, 163, 184, 0.18)) 36%, transparent)",
  borderRadius: MAP_RADIUS.sm,
  background: "color-mix(in srgb, var(--syn-surface-subtle, rgba(15, 23, 42, 0.2)) 20%, transparent)",
  gap: "0.125rem",
};
...
const compactCommandCenterRail: React.CSSProperties = {
  display: "inline-flex",
  alignItems: "center",
  flex: "0 0 auto",
  gap: "0.125rem",
...
const activeLensShell: React.CSSProperties = {
  display: "inline-flex",
  alignItems: "center",
  flex: "0 0 auto",
  minWidth: MAP_SPACING.zero,
...
const topSurfaceGroupRail: React.CSSProperties = {
  display: "inline-flex",
  alignItems: "center",
  flex: "0 0 auto",
  gap: "0.125rem",
...
const overflowRail: React.CSSProperties = {
  ...
  borderLeft: "1px solid color-mix(in srgb, var(--syn-border-subtle, rgba(148, 163, 184, 0.18)) 40%, transparent)",
};
...
const menuHeaderStyle: React.CSSProperties = {
  ...
  borderBottom: "1px solid color-mix(in srgb, var(--syn-border-subtle, rgba(148, 163, 184, 0.28)) 52%, transparent)",
...
function commandButtonStyle(
  ...
  const border = primary
    ? `1px solid color-mix(in srgb, ${MAP_COLORS.interaction} 36%, transparent)`
...
function pickCommandsById(commands: readonly ToolbarCommand[], ids: readonly string[]): ToolbarCommand[] {
  const byId = new Map(commands.map((command) => [command.id, command]));
  return ids
    .map((id) => byId.get(id))
    .filter((command): command is ToolbarCommand => command != null);
}
...
export function getVisibleTopSurfaceGroupIds(width: number): readonly TopSurfaceGroupId[] {
...
export function groupCommandsByOverflowSection(commands: readonly ToolbarCommand[]): Array<{
  id: ToolbarMenuSectionId;
  label: string;
  commands: ToolbarCommand[];
}> {
  const sectionOrder: readonly ToolbarMenuSectionId[] = ["workspace", "view", "tools", "settings"];
  const labels: Record<ToolbarMenuSectionId, string> = {
    workspace: "Workspace",
    view: "View",
    tools: "Tools",
    settings: "Settings",
  ...
  return sectionOrder.map((id) => ({
    id,
    label: labels[id],
    commands: sortCommandsForSurface(commands.filter((command) => getOverflowSectionId(command) === id)),
  })).filter((section) => section.commands.length > 0);
}

export function ToolbarCommandsMenu({
  commands,
  density,
  open,
  onOpenChange,
    ...
    <AppDropdownMenu
      ...
      trigger={(
        <ToolbarMenuButton
          ...
          title="Open map commands"
          ariaLabel="Open map commands"
          testId="map-commands-trigger"
        />
      )}
    >
      ...
      <AppMenuSection title="Quick Actions">
        <AppMenuItem
          ...
          shortcut={formatMapKeybinding("openPalette")}
          onSelect={() => {
...
export function TaskLensSwitch({
  taskLens,
  onTaskLensChange,
  compact = false,
}: {
    ...
    >
      {MAP_RUNTIME_TASK_LENSES.map((entry) => (
        ...
        >
          {compact ? (
            <>
              <span aria-hidden>{TASK_LENS_MICRO_LABELS[entry.id]}</span>
              <span style={mapStyles.srOnly}>{TASK_LENS_SHORT_LABELS[entry.id]}</span>
            </>
...
export function ToolbarOverflowMenu({
  ...
  const groupedCommands = groupCommandsByOverflowSection(commands);
  ...
  return (
    <AppDropdownMenu
      ...
      trigger={(
        <ToolbarMenuButton
          label="More"
          icon={<Settings2 size={MAP_ICON_SIZES.sm} strokeWidth={1.8} aria-hidden="true" />}
          active={open}
          expanded={open}
          compact={density === "compact"}
          title="Open overflow map commands"
          ariaLabel="Open overflow map commands"
          testId="map-command-center-overflow"
        />
      )}
    >
      <div>
...
export function ToolbarCommandGroupMenu({
  groupId,
  commands,
  density,
  open,
  ...
  return (
    <>
      ...
      <AppPopover
        ...
        testId={`map-command-group-menu-${groupId}`}
      >
        <div>
...
export const MapToolbar: React.FC<MapToolbarProps> = ({
  ...
  const [openMenu, setOpenMenu] = React.useState<TopSurfaceGroupId | "commands" | "more" | null>(null);
  ...
  const { menus, quickActions } = React.useMemo(
    () => buildMapPremiumMenuModel({
      commands: commandRegistry,
      onOpenPalette: () => {
        setPaletteOpen(true);
        setOpenMenu(null);
      },
      paletteShortcut: openPaletteShortcut,
      primaryCommand,
    }),
    [commandRegistry, openPaletteShortcut, primaryCommand],
  );
  const visibleTopSurfaceGroupIds = React.useMemo(() => getVisibleTopSurfaceGroupIds(toolbarWidth), [toolbarWidth]);
  const topSurfaceCommandGroups = React.useMemo(() => {
    const groups = {} as Record<TopSurfaceGroupId, ToolbarCommand[]>;
    for (const groupId of Object.keys(TOP_SURFACE_GROUP_COMMAND_IDS) as TopSurfaceGroupId[]) {
      groups[groupId] = pickCommandsById(commandRegistry, TOP_SURFACE_GROUP_COMMAND_IDS[groupId]);
    }
    return groups;
  }, [commandRegistry]);
  const overflowCommands = React.useMemo(
    () => pickCommandsById(commandRegistry, COMMAND_CENTER_OVERFLOW_COMMAND_IDS),
    [commandRegistry],
  );
  ...
  const visibleCommandCount =
    menus.length +
    quickActions.length +
    visibleTopSurfaceGroupIds.length +
  ...
  return (
    ...
    >
      <div style={{ ...commandRail, gap: MAP_SPACING.xs }} aria-label="Premium menu bar and contextual quick actions">
        ...
        <div style={{ ...compactCommandCenterRail, display: "none" }} aria-label="Compact command-center controls" aria-hidden="true">
          <div
            ...
            data-primary-command-id={primaryCommand?.id ?? undefined}
          >
            {primaryCommand ? (
              <ToolbarCommandButton command={primaryCommand} density={density} primary />
            ) : null}
          </div>
          <ToolbarCommandsMenu
            commands={commandRegistry}
            density={density}
            open={openMenu === "commands"}
            onOpenChange={(open) => setOpenMenu(open ? "commands" : null)}
            onOpenPalette={() => {
              setPaletteOpen(true);
              setOpenMenu(null);
            }}
          ...
          >
            {visibleTopSurfaceGroupIds.map((groupId) => (
              <ToolbarCommandGroupMenu
                key={groupId}
                groupId={groupId}
                commands={topSurfaceCommandGroups[groupId]}
                density={density}
                open={openMenu === groupId}
                onToggle={() => setOpenMenu((current) => (current === groupId ? null : groupId))}
                onClose={() => setOpenMenu(null)}
                showLabel={showLegacyGroupLabels}
              />
            ))}
          </div>
          <ToolbarOverflowMenu
      ...
      {workspaceView === "analyze" || workspaceView === "navigator" ? (
        <div style={overflowRail} aria-label="Background map tasks">
          <BackgroundTasksControl compact />
        </div>
      ) : null}
      ...
      <span style={mapStyles.srOnly}>
        The premium menu bar exposes Project, Add Data, Layers, Style, Analyze, Scene, Publish, Review, Controls, Plugins, View, and Help menus. The command palette keeps the complete map command registry searchable.
      </span>
      <span style={mapStyles.srOnly} data-testid="map-command-registry-summary">
        {commandRegistry.map((command) => getCommandAccessibleTitle(command)).join(". ")}
      </span>
    </div>
  );
};
