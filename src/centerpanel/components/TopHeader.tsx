...
const TAB_ICONS: Record<string, React.ComponentType> = {
  "Projects": LuDatabase,
  "New Project": LuUserPlus,
  "Methods": LuBookOpen,
  "Education": LuActivity,
  "Report": LuFileEdit,
...
const TopHeader: React.FC<TopHeaderProps> = ({ tabs, activeTab, onTabChange, sessionLabel, nowTs, children }) => {
  ...
  const headerClass = `${hdr.header} ${hdr.headerShowRight}`;
  const statusIndicatorClass = `${hdr.statusIndicator} ${hdr[`status_${systemStatus}`]}`;
  
  return (
    <header className={headerClass} role="banner" data-transition={animateTransition ? 'active' : undefined} data-density="compact">
      ...
      <div className={hdr.row}>
        <div className={hdr.brand}>
          ...
          <div className={hdr.brandText}>
            <div className={hdr.brandLine}>
              <span className={hdr.clinician}>Analyst</span>
        ...
        <div className={hdr.tabsWrap} role="tablist" aria-label="Center panel navigation tabs" onKeyDown={handleTablistKeyDown} tabIndex={0}>
          {tabs.map((tab, i) => {
            ...
            const shortLabel = TAB_SHORT_LABELS[tab] ?? tab;
            return (
              <button
                key={tab}
                role="tab"
                className={hdr.tab}
                data-testid={`cp-tab-${tab.toLowerCase().replace(/\s+/g, '-')}`}
              ...
              >
                ...
                <span className={hdr.tabLabel}>
                  <span className={hdr.tabLabelFull}>{tab}</span>
                  <span className={hdr.tabLabelShort} aria-hidden="true">{shortLabel}</span>
                </span>
      ...
      {}
      {(activeTab === "Methods" || activeTab === "Workflows" || activeTab === "Dashboard") ? null : (
