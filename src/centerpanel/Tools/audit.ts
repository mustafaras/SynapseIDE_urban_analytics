...
export type ScopeKind = "session" | "project" | "cohort";
...
export function snapshotSelection(state: RegistryState, scopeKind: ScopeKind): SelectionSnapshot {
  ...
  if (scopeKind === "session") {
    ...
    if (s.selectedProjectId) out.projectId = s.selectedProjectId;
    if (s.selectedSessionId) out.sessionId = s.selectedSessionId;
    return out;
  }
  ...
  const cohort = filterProjects(s);
  const out: SelectionSnapshot = { projectIds: cohort.map((p: any) => p.id) };
  out.filter = s.filter;
  return out;
}
...
function reconstructScopeFromSnapshot(state: RegistryState, rec: AuditRecord) {
  ...
  if (kind === "session") {
    const project = (s.projects || []).find((p: any) => p.id === rec.selection.projectId);
    const enc = ((project as any)?.sessions ?? (project as any)?.encounters)?.find((e: any) => e.id === rec.selection.sessionId);
    return { scopeKind: "session", scopeLabel: rec.scopeLabel, sessions: enc ? [enc] : [], projects: [] };
  }
  if (kind === "project") {
    const project = (s.projects || []).find((p: any) => p.id === rec.selection.projectId);
    return { scopeKind: "project", scopeLabel: rec.scopeLabel, projects: project ? [project] : [], sessions: [] };
  }

  const ids = new Set(rec.selection.projectIds || []);

  const projects = (s.projects || [])
    .filter((p: any) => ids.has(p.id))
    .slice()
    .sort((a: any, b: any) => (a.id > b.id ? 1 : a.id < b.id ? -1 : 0));
  return { scopeKind: "cohort", scopeLabel: rec.scopeLabel, projects: projects, sessions: [] };
}
