...
import { type ExternalExportPayload, publishExternalExport } from "./exportInbox";
import { snapshotSelection } from "./audit";

type Tab = "pdf" | "json" | "csv";

interface Props {
  scopeKind: "session" | "project" | "cohort";
  policy: DeidPolicy;
...
function validateJson(jsonText: string) {
  try {
    ...
    const projects: unknown[] =
      parsed?.kind === "registry.v1" && Array.isArray(parsed.projects ?? parsed.patients)
        ? (parsed.projects ?? parsed.patients)
    ...
    return { ok: true, rows: projects.length, missingId, missingRisk };
  } catch (e) {
...
const EMPTY_NOTICE = "No selection for the chosen scope. Choose a Session, Project, or a non-empty Cohort.";
...
const PreviewPanel: React.FC<Props> = ({ scopeKind, policy, csvPreviewRows = 25, debounceMs = 150 }) => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (cancelled) return;
      try {
        const scope = resolveScope(state as unknown as RegistryState, scopeKind);
        const empty = !((scope.projects?.length ?? 0) > 0 || (scope.sessions?.length ?? 0) > 0);
        setIsEmptyScope(empty);
        const out = assembleForPreview(state as unknown as RegistryState, scope, policy);
        if (cancelled) return;
      ...
      <div className={styles.hstack} style={{ justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
        ...
        <button
          ...
          onClick={() => {
            try {
              const scope = resolveScope(state as unknown as RegistryState, scopeKind);
              const selection = snapshotSelection(state as unknown as RegistryState, scopeKind);
