...
function getAlias(p: ProjectRecord): string | undefined {
...
function scoreProject(p: ProjectRecord, terms: string[], isPinned: boolean): number {
  let s = 0;
  ...
  s += Math.min(5, Math.max(0, (p.priority ?? 0) - 1));
  const last = p.lastSessionDate ? Date.parse(p.lastSessionDate) : 0;
  if (last) {
...
const ToolsProjectList: React.FC = () => {
  ...
  const rows = React.useMemo(() => {
    let list = (state.projects ?? []) as ProjectRecord[];
    const spec = parseQuery(query);


    if (cohort === "Low") list = list.filter(p => (p.priority ?? 0) >= 1 && (p.priority ?? 0) <= 2);
    if (cohort === "Moderate") list = list.filter(p => (p.priority ?? 0) === 3);
    if (cohort === "High") list = list.filter(p => (p.priority ?? 0) >= 4);
    if (cohort === "Recent") {
      list = list
        .slice()
        .sort((a, b) => ((b.lastSessionDate ? Date.parse(b.lastSessionDate) : 0) - (a.lastSessionDate ? Date.parse(a.lastSessionDate) : 0)))
        .slice(0, 30);
    }
    if (cohort === "Pinned") {
    ...
    if (spec.risk) {
      if (spec.risk === "low") list = list.filter(p => (p.priority ?? 0) >= 1 && (p.priority ?? 0) <= 2);
      if (spec.risk === "moderate") list = list.filter(p => (p.priority ?? 0) === 3);
      if (spec.risk === "high") list = list.filter(p => (p.priority ?? 0) >= 4);
    }
    if (spec.id) {
    ...
    if (spec.textTerms.length || spec.risk || spec.pinned || spec.id) {
      const withScore = list.map(p => ({ p, s: scoreProject(p, spec.textTerms, pinned.has(String(p.id))) }));
      withScore.sort((a, b) => b.s - a.s);
      list = withScore.map(x => x.p);
    } else if (effectiveSort === "RECENT") {
      list = list.slice().sort((a, b) => ((b.lastSessionDate ? Date.parse(b.lastSessionDate) : 0) - (a.lastSessionDate ? Date.parse(a.lastSessionDate) : 0)));
    } else if (effectiveSort === "RISK") {
      list = list.slice().sort((a, b) => ((b.priority ?? 0) - (a.priority ?? 0)));
    } else if (effectiveSort === "AZ") {
      list = list.slice().sort((a, b) => (((getAlias(a) || a.name || "").localeCompare((getAlias(b) || b.name || "")))));
    }

    return list;
  }, [state.projects, cohort, query, sortBy, pinned]);


  React.useEffect(() => {
    const idx = rows.findIndex(p => p.id === state.selectedProjectId);
    activeRef.current = idx >= 0 ? idx : 0;
  }, [rows, state.selectedProjectId]);
  ...
  React.useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if ((e.key === "e" || e.key === "E") && !typing) {
        e.preventDefault();
        if (current) {
          actions.selectProject(current.id);
      ...
      if (e.key === "ArrowDown" && !typing) {
        ...
        actions.selectProject(next.id);
        return;
      }
      if (e.key === "ArrowUp" && !typing) {
        e.preventDefault();
        const prev = rows[Math.max(0, idx - 1)] ?? rows[rows.length - 1];
        actions.selectProject(prev.id);
        return;
      }
      if (e.key === "Home" && !typing) {
        e.preventDefault();
        actions.selectProject(rows[0].id);
        return;
      }
      if (e.key === "End" && !typing) {
        e.preventDefault();
        actions.selectProject(rows[rows.length - 1].id);
        return;
      }
      ...
      if (e.key === "PageUp" && !typing) {
        ...
        actions.selectProject(prev2.id);
      }
    };
    document.addEventListener("keydown", handler);
    return () => document.removeEventListener("keydown", handler);
  }, [rows, state.selectedProjectId, cohort, actions, togglePin]);
  ...
  const totalCount = (state.projects?.length ?? 0);
  const pinnedCount = (state.projects ?? []).filter(p => pinned.has(String(p.id))).length;
  const highPriorityCount = (state.projects ?? []).filter(p => (p.priority ?? 0) >= 4).length;

  return (
  <div className={`${lcss.wrap}`} role="region" aria-label="Tools project list" ref={wrapRef}>
      {}
      <div className={lcss.headerRow}>
        <div className={lcss.headerTop}>
          <div className={lcss.headerTitle}>Projects</div>
          <div className={lcss.headerStats}>
            <div className={lcss.statCard} data-type="total" title="Total projects">
              <div className={lcss.statValue}>{totalCount}</div>
              <div className={lcss.statLabel}>Total</div>
            </div>
            {highPriorityCount > 0 && (
              <div className={lcss.statCard} data-type="high" title="High priority projects">
                <div className={lcss.statValue}>{highPriorityCount}</div>
                <div className={lcss.statLabel}>High Priority</div>
              </div>
            )}
            {pinnedCount > 0 && (
              <div className={lcss.statCard} data-type="pinned" title="Pinned projects">
  ...
  {}
      ...
      {}
      <div className={lcss.chipRow} role="tablist" aria-label="Project cohorts">
        {(["All","Low","Moderate","High","Recent","Pinned"] as Cohort[]).map(c => (
          <button
            ...
            onClick={() => setCohort(c)}
            title={
              c === "All" ? "Show all projects" :
              c === "Low" ? "Low priority cohort" :
              c === "Moderate" ? "Moderate priority cohort" :
              c === "High" ? "High priority cohort" :
      ...
      {rows.length > 150 ? (
        <VirtualList
          ...
          renderRow={(p) => {
            ...
            const lastWhen = p.lastSessionDate ? Date.parse(p.lastSessionDate) : undefined;
            ...
            const prioNum = (p.priority ?? 0) as number;
            const prioLabel = prioNum >= 4 ? "High" : prioNum === 3 ? "Moderate" : prioNum >= 1 ? "Low" : "";
            return (
              <div
              ...
              >
                ...
                <div className={lcss.itemRight}>
                  {prioNum > 0 && (
                    <span className={lcss.riskChip} data-risk={prioNum} title={`Priority: ${prioLabel}`} aria-label={`Priority: ${prioLabel}`}>
                      {prioLabel}
                    </span>
                  )}
      ...
      {rows.length === 0 && (
        <div className={lcss.empty}>No projects match this filter. Try adjusting the search term or clearing the filter to see all available projects.</div>
      )}
    </div>
  );
};

export default ToolsProjectList;
