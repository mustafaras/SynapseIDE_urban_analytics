...
function countForScope(state: RegistryState, scopeKind: ScopeKind): number {
  if (scopeKind === "cohort") return filterProjects(state as any).length;
  if (scopeKind === "session") {
    if ((state as any).selectedSessionId) return 1;
    const p = ((state as any).projects ?? []).find((pp: any) => pp.id === (state as any).selectedProjectId);
    return ((p as any)?.sessions?.length ?? 0) > 0 ? 1 : 0;
  }
  if (scopeKind === "project") {
    return (state as any).selectedProjectId ? 1 : 0;
  }
  return 0;
}
...
const ExportBar: React.FC<ExportBarProps> = ({ scopeKind, policy }) => {
  const { state: rawState } = useProjectRegistry();
  const state = rawState as unknown as RegistryState;
    ...
    <div role="region" aria-label="Export actions" className={styles.vstack} style={{ gap: 8 }}>
      {external ? (
        <div
          ...
          aria-live="polite"
        >
          <div className={styles.externalBannerIcon} aria-hidden="true">i</div>
      ...
      <div className={styles.meta} style={{ maxWidth: "72ch" }}>
        <em>Exports support professional communication and documentation. They are not directives or orders.</em>
      </div>
              ...
              <li key={rec.id}>
                <div className={styles.recentItem}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        const { out, matches } = await rerunAndChecksum(state as unknown as RegistryState, rec);
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const { out } = await rerunAndChecksum(state as unknown as RegistryState, rec);
