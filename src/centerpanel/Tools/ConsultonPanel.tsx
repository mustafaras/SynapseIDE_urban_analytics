...
import { extractFirstFencedBlock, reduceForExport, tryParseJson } from "./lib/markdown";
...
const ConsultonPanel: React.FC<Props> = ({ modelLabel = "GPT-4o" }) => {
  const { t } = useI18n();
  const { state: appState } = useProjectRegistry();
  ...
  useEffect(() => {
    const onKey = async (e: KeyboardEvent) => {
      ...
      if (e.code === "KeyD") {
        ...
        const reduced = reduceForExport(src, { max: 1400 });
  ...
  const ctxObj = assembleConsultContext(appState as any, effectivePolicy, 50, { redact });
    ...
    if ((ctxObj as any).scopeKind === "empty" || (((ctxObj as any).items?.length) ?? 0) === 0) {
      ...
      setStatus("idle");
      politeAnnounce(setSrMsg, "Empty scope. Select a project, session, or cohort.");
      taRef.current?.focus();
      return;
    }
  ...
  const StatusBlock: React.FC<{ notice: UiNotice; onClose?: () => void; onRetry?: () => void; onOpenSettings?: () => void; }> = ({ notice, onClose, onRetry, onOpenSettings }) => {
    ...
    if (notice.kind === "empty_scope") {
      ...
      body = <>Select a <strong>project/session</strong> or apply a <strong>cohort filter</strong> to assemble a consult context.</>;
      actions = (
  ...
  {}
      ...
      <div className={styles.consultHeader} role="region" aria-label={`${BRAND_NAME} header`}>
        ...
        <div className={styles.headerRight}>
          ...
          >
            Settings<span className={styles.srOnly}>Open settings</span>
          </button>
          {}
          <button
            ref={helpBtnRef}
          ...
          {!!helpOpen && (
            <div role="dialog" aria-label={t("help.title")} className={styles.helpPopover}>
              ...
              <div className={styles.meta}>
                <ul>
                  <li>{t("help.analystPoint1")}</li>
                  <li>{t("help.analystPoint2")}</li>
                  <li>{t("help.analystPoint3")}</li>
                </ul>
      ...
      <div role="note" aria-live="polite" className={styles.consultHeaderNote || undefined}>
        Supports documentation, not directives. Always apply analytical judgment.
      </div>
      ...
      <div className={styles.consultEditor} role="region" aria-label="Prompt editor">
        ...
        <textarea
          ...
          placeholder="Ask a focused, analytically appropriate question…"
          value={prompt}
            ...
            <div className={styles.outActions}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const scopeKind: "session" | "project" | "cohort" = (ctxObj?.scopeKind === "session" || ctxObj?.scopeKind === "project") ? ctxObj.scopeKind : "cohort";
              ...
              <button
                ...
                aria-label="Copy reduced summary"
              >
                <ScissorsIcon />
              </button>
              {}
