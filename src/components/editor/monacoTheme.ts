...
export const defineSynapseMonacoTheme = (monaco: typeof monacoNs) => {
  ...
  const bg = SYNAPSE_COLORS?.bgDark ?? '#000000';
  const panel = SYNAPSE_COLORS?.bgSecondary ?? '#121212';
  const border = SYNAPSE_COLORS?.border ?? '#23262d';
  const muted = SYNAPSE_COLORS?.textSecondary ?? '#A8A29E';
  const text = SYNAPSE_COLORS?.textPrimary ?? '#FAFAF9';
  ...
  const info = SYNAPSE_COLORS?.accentNeutral ?? '#f59e0b';
  const add = '#2e7d32';
  const del = '#c62828';
