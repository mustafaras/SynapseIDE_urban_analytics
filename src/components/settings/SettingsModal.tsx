...
const TabPanel = styled.div<{ $active?: boolean }>`
  ...
  inset: 0;
  transition: opacity 160ms var(--syn-easing-bauhaus);
...
export const SettingsModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => {
  ...
  const spatialIndexWasmEnabled = useSpatialIndexStore((state) => state.enableWasm);
  const setSpatialIndexWasmEnabled = useSpatialIndexStore((state) => state.setEnableWasm);
  const spatialIndexBackend = useSpatialIndexStore((state) => state.backend);
  const spatialIndexStatus = useSpatialIndexStore((state) => state.state);
  const spatialIndexRecords = useSpatialIndexStore((state) => state.records);
  const spatialIndexLastQueryKind = useSpatialIndexStore((state) => state.lastQueryKind);
  const spatialIndexLastQueryMs = useSpatialIndexStore((state) => state.lastQueryMs);
  const spatialIndexLastSpeedup = useSpatialIndexStore((state) => state.lastSpeedup);
          ...
          <TabsContentWrap>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  ...
                  <Button
                    ...
                    style={{ alignSelf:'flex-start', background:'var(--syn-danger-bg, rgba(239,68,68,0.12))', borderColor:'var(--borderSoft)' }}
              ...
              <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
                <Label style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  {defaultProv === currentProvider && (
                    <span style={{ fontSize:10, padding:'2px 6px', border:'1px solid var(--borderSoft)', borderRadius:6, color:'var(--success)', background:'var(--syn-success-bg, rgba(34,197,94,0.12))' }}>Applied</span>
                  )}
                </Label>
            ...
            <TabPanel role="tabpanel" id={`${id}-panel-advanced`} aria-labelledby={`${id}-tab-advanced`} $active={tab==='advanced'} data-hidden={tab!=='advanced'}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8, background:'rgba(255,255,255,0.03)', padding:'8px 10px', border:'1px solid var(--borderSoft)', borderRadius:8 }}>
                  <Label style={{ fontSize:11, textTransform:'uppercase', letterSpacing:0.5 }}>Spatial Index Acceleration</Label>
                  <label style={{ display:'inline-flex', alignItems:'center', gap:8, fontSize:12 }}>
                    <input type="checkbox" checked={spatialIndexWasmEnabled} onChange={e => setSpatialIndexWasmEnabled(e.target.checked)} /> Enable WebAssembly backend for bbox and nearest-neighbor queries
                  </label>
                  <Hint>
                    Status: {spatialIndexStatus} · Backend: {spatialIndexBackend === 'wasm' ? 'WASM' : 'JS fallback'} · Records: {spatialIndexRecords.toLocaleString()}
                    {typeof spatialIndexLastQueryMs === 'number' ? ` · Last ${spatialIndexLastQueryKind ?? 'query'} ${spatialIndexLastQueryMs.toFixed(1)} ms` : ''}
                    {typeof spatialIndexLastSpeedup === 'number' ? ` · ${spatialIndexLastSpeedup.toFixed(2)}x over baseline` : ''}
                  </Hint>
                </div>
...
const ResetSection: React.FC<ResetProps> = ({ clearAiKey, setUI, setTokenBudget, setAdvanced }) => {
  const setDefaults = useAiSettingsStore(s => s.setDefaults);
  const setContext = useAiSettingsStore(s => s.setContext);
  const defaultsGen = { provider: 'openai' as const, model: 'gpt-5-mini', temperature: 0.2, maxTokens: 2048 };
