...
export const KeyDebug: React.FC = () => {
  ...
  st.setDefaults(patch);
        ...
        try { const { emitAiRouteChanged } = require('@/stores/useAiSettingsStore'); emitAiRouteChanged(before, { provider: nextProv, model: uModel }); } catch {} // eslint-disable-line @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires
      }
    } catch {}
  }, [uProv, uModel]);
  ...
  return (
    <div style={{ fontFamily: 'var(--font-code, monospace)', fontSize: 11, opacity: 0.55, padding: '4px 8px', borderTop: '1px solid var(--syn-overlay-whisper)' }}>
