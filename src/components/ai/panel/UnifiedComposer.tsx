...
import editorBridge, { type SupportedLang } from '@/services/editorBridge';
...
import ApplyPlanPreview from '@/components/ai/apply/ApplyPlanPreview';
...
import { clampSnapshot, useApplyHistoryStore } from '@/stores/useApplyHistoryStore';
...
const UnifiedComposer: React.FC<UnifiedComposerProps> = ({
  ...
  const [previewPlan, setPreviewPlan] = useState<ApplyPlan | null>(null);
  ...
  const legacyModel = (profileData?.settings?.model || profileData?.model || 'gpt-5-mini');
  ...
  const executeApplyPlanHelper = useCallback((plan: ApplyPlan) => {
    try {
      const editorState = useEditorStore.getState();
      const fileState = useFileExplorerStore.getState();
      const existingPaths = new Set(editorState.tabs.map(t => t.path).filter(Boolean) as string[]);

      const result = executeApplyPlan(plan, {
        fileExists: (p) => existingPaths.has(p),
        readFile: (p) => editorState.tabs.find(t => t.path === p)?.content ?? null,
        createFile: (p, content, monacoLang) => {
          const existing = editorState.tabs.find(t => t.path === p);
          if (existing) {
            editorState.updateTabContent(existing.id, content);
          } else {
            const fid = `file-${Date.now()}-${Math.random().toString(36).slice(2)}`;
            const name = p.split('/').pop() || p;
            fileState.addFile({ id: fid, name, type: 'file', path: p, content, language: monacoLang, lastModified: new Date(), size: content.length }, '/');
            editorState.openTab({ id: fid, name, type: 'file', path: p, content, language: monacoLang, lastModified: new Date(), size: content.length });
          }
        },
        replaceFile: (p, content) => {
          const tab = editorState.tabs.find(t => t.path === p);
          if (tab) editorState.updateTabContent(tab.id, content);
        },
        insertIntoActive: (content) => {
          const maybeLanguage = plan.items[0]?.monaco as SupportedLang | undefined;
          void editorBridge.insertIntoActive(
            maybeLanguage ? { code: content, language: maybeLanguage } : { code: content }
          );
        },
        setActiveTab: (p) => { const tab = editorState.tabs.find(t => t.path === p); if (tab) editorState.setActiveTab(tab.id); },
        pushUndoSnapshot: (p, prevContent) => {
          const tab = editorState.tabs.find(t => t.path === p);
          if (tab) editorState.addToHistory(tab.id, prevContent, tab.cursorPosition || { line: 1, column: 1 });
        },
      }, { preferInsertIntoActive: true });

      // ── Push to apply history ────────────────────────────────────────────
      useApplyHistoryStore.getState().pushRecord({
        id: plan.id,
        createdAt: plan.createdAt,
        appliedAt: new Date().toISOString(),
        ...(plan.sourcePrompt ? { sourcePrompt: plan.sourcePrompt.slice(0, 200) } : {}),
        status: plan.status,
        files: plan.items.map((item) => {
          const snapshot =
            item.action === 'replace' && item.accepted !== false
              ? clampSnapshot(item.originalContent)
              : undefined;
          return {
            path: item.path,
            action: item.action,
            accepted: item.accepted !== false,
            ...(snapshot ? { revertSnapshot: snapshot } : {}),
          };
        }),
        result: { applied: result.applied, rejected: result.rejected, failed: result.failed },
        conflicts: [
          ...plan.conflicts.map((c) => ({ path: c.path, reason: c.message })),
          ...result.conflicts,
        ],
      });

      // ── Push lightweight ref to .synapse/apply-history.json ────────────
      useSynapseWorkspaceStore.getState().pushApplyHistoryRef({
        id: plan.id,
        appliedAt: new Date().toISOString(),
        status: plan.status,
        fileCount: plan.items.length,
        ...(plan.sourcePrompt ? { promptSnippet: plan.sourcePrompt.slice(0, 120) } : {}),
      });

      const toastMsg = result.applied > 0
        ? `Applied ${result.applied} file${result.applied !== 1 ? 's' : ''} (undo available)`
        : 'No files applied';
      showToast({ kind: result.applied > 0 ? 'success' : 'warning', message: toastMsg });
    } catch (_err) {
      showToast({ kind: 'error', message: `Apply failed: ${_err instanceof Error ? _err.message : String(_err)}` });
    }
  }, []);

  const onInsertLast = useCallback(() => {
    if (!lastAssistant) return;
    const blk = extractFirstCode(lastAssistant.content); if (!blk) return;
    const { monaco } = mapFenceToLangAndExt(blk.info);
    const langId = monaco || 'plaintext';
    try {
      const editorState = useEditorStore.getState();
      const existingPaths = new Set(editorState.tabs.map(t => t.path).filter(Boolean) as string[]);
      const activeTab = editorState.tabs.find(t => t.id === editorState.activeTabId);
      const plan = buildApplyPlan({
        rawAssistantText: `\`\`\`${langId}\n${blk.code}\n\`\`\``,
        selectedLanguageId: langId,
        mode: 'beginner',
        defaultDir: activeTab?.path ? activeTab.path.split('/').slice(0, -1).join('/') : '',
        existingPaths,
      });

      // Always show preview so user can review before applying
      setPreviewPlan(plan);
    } catch (_err) {
      const blk_fallback = extractFirstCode(lastAssistant.content);
      if (blk_fallback) {
        void editorBridge.insertIntoActive({ code: blk_fallback.code, language: langId as SupportedLang });
      }
    }
  }, [lastAssistant, extractFirstCode, executeApplyPlanHelper]);
        ...
        <div style={{
          ...
          <ApplyPlanPreview
            ...
            onApply={(plan) => {
              ...
              setPreviewPlan(null);
              executeApplyPlanHelper(plan);
              setIsApplyingFromPreview(false);
            }}
            onCancel={() => {
              setPreviewPlan(null);
            }}
            isApplying={isApplyingFromPreview}
          />
        </div>
      </div>
    ) : null}
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <QuickActionsBar aria-label="Composer Actions" style={{ padding: '4px 8px' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <IconButton aria-label="Apply first code block (with undo)" title="Apply first code block (with undo)" disabled={!hasCode} onClick={onInsertLast}><ClipboardPaste size={16} /></IconButton>
