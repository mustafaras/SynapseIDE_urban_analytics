...
/* Local palette redirected to the semantic VS Code blue interaction token chain.
   `gold*` key names retained for source compatibility; values resolve to blue. */
const COLORS = {
  border: '#3A3A3A',
  divider: '#2A2A2A',
  textPrimary: '#FAFAF9',
  ...
  goldSecondary: 'color-mix(in srgb, var(--syn-interaction-active) 80%, var(--syn-surface-workbench) 20%)',
...
const useHeaderStyles = (sidebarWidth: number = 280) => ({
  ...
  searchInput: {
    ...
    boxShadow: 'none',
    '&:focus': {
      borderColor: 'var(--syn-border-focus)',
      background: 'var(--syn-surface-input)',
      boxShadow: 'none',
    },
  },
  ...
  clearButton: {
    ...
    borderRadius: '2px',
    transition: getTransition('color, background'),
    opacity: 1,
    '&:hover': {
      color: 'var(--syn-text-default)',
    ...
    '&:active': {
      transform: 'translateY(-50%)',
      transition: getTransition('color, background', MOTION.duration.instant),
    },
  },
  ...
  actionButton: {
    ...
    '&:active': {
      transform: 'none',
      transition: getTransition('background, color', MOTION.duration.instant),
    },
    '&:focus': {
      outline: '1px solid var(--syn-interaction-focus-ring)',
      outlineOffset: '-1px',
    },
  },
  ...
  primaryButton: {
    ...
    border: 'none',
    borderRadius: '2px',
    color: 'color-mix(in srgb, var(--syn-interaction-active) 78%, var(--syn-text-muted) 22%)',
    fontSize: TYPOGRAPHY.fontSize.small,
    fontFamily: TYPOGRAPHY.fontFamily,
    fontWeight: TYPOGRAPHY.fontWeight.medium,
    cursor: 'pointer',
              ...
              {exportStatus ? <div
                  style={{
                    ...
                    color: exportStatus.includes('[OK]')
                      ? 'var(--syn-status-valid)'
                      : exportStatus.includes('[ERR]')
                        ? 'var(--syn-status-error)'
                        : 'var(--syn-status-info)',
