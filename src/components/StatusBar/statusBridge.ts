...
/* ── GeoAI Engine status ────────────────────────────── */
export const sbGeoAiLoading = () =>
  mergeEmit('geoai', { state: 'loading' } as any);
export const sbGeoAiReady = (loadedModels: number, memoryUsedMB: number, backend: 'wasm' | 'webgpu' = 'wasm') =>
  mergeEmit('geoai', { state: 'ready', loadedModels, memoryUsedMB, backend } as any);
export const sbGeoAiInferring = () =>
  mergeEmit('geoai', { state: 'inferring' } as any);
export const sbGeoAiError = () =>
  mergeEmit('geoai', { state: 'error' } as any);
export const sbGeoAiIdle = () =>
  mergeEmit('geoai', { state: 'idle', loadedModels: 0, memoryUsedMB: 0, backend: 'wasm' } as any);

export const sbStreaming = (update: Partial<NonNullable<StatusSnapshot['streaming']>>) =>
  mergeEmit('streaming', update as any);

export const sbSpatialIndex = (update: Partial<NonNullable<StatusSnapshot['spatialIndex']>>) =>
  mergeEmit('spatialIndex', update as any);
