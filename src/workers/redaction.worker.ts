...
const IDLIKE = /\b(?:ID|Record|Ref)\s*[:#-]?\s*([A-Z0-9]{3,}[- ]?[A-Z0-9]{2,})\b/gi;
...
const NAME_HINT = /\b(?:Name|Full\s*Name)\s*[:#-]?\s*[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\b/g;
...
function scrubDeep(value: unknown, opts: RedactOptions): unknown {
  ...
  if (t === 'object') {
    ...
    for (const key of Object.keys(src)) {
      ...
      if (opts.categories.name && /^(name|fullname|firstname|lastname)$/.test(norm)) { out[key] = '[REDACTED_NAME]'; continue; }
      if (opts.categories.ids && /^(id|recordid|projectid|ref)$/i.test(norm)) { out[key] = '[REDACTED_ID]'; continue; }
