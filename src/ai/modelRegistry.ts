...
import { OPENAI_CHAT_DEFAULT_MODELS } from '../utils/ai/models/openaiGpt5';
...
export async function listModelsDynamic(p: ProviderId, keys: ProviderKey): Promise<string[]> {
  ...
  const now = Date.now();
  const ttl = 30 * 60_000; // 30 minutes in-memory cache
...
export const __MODEL_REGISTRY_VERSION = 7;
