...
const buildOpenAI: ProviderRequestBuilder = ({ model, sampling, apiKey, baseUrl, prompt }, opts) => {
  ...
  const useResponses = isUsableOpenAIGpt5Model(model);
  const supportsSampling = openAIModelSupportsSampling(model);
  const url = `${(baseUrl || OPENAI_BASE).replace(/\/$/,'')}${useResponses ? '/responses' : '/chat/completions'}`;
  const body: any = useResponses
    ? {
        model,
        input: messages.map((message) => ({ type: 'message', role: message.role, content: message.content })),
    ...
    : {
        model,
        messages,
        ...(supportsSampling ? { temperature: sampling.temperature } : {}),
        max_tokens: sampling.max_tokens,
        stream: opts?.stream ?? true,
      };
  if (!useResponses && supportsSampling && sampling.top_p != null) body.top_p = sampling.top_p;
  if (sampling.json_mode && !useResponses) body.response_format = { type: 'json_object' };
  return {
    provider: 'openai',
    model,
    request: {
