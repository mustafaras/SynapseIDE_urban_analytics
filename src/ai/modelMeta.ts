...
export function deriveModelMeta(provider: ProviderId, id: string): ModelMeta {
  ...
  if (provider === 'openai') {
    if (lower.startsWith('gpt-5')) family = 'gpt-5';
    else if (lower.startsWith('gpt-4o')) family = 'gpt-4o';
  ...
  if (provider === 'openai') {
    if (lower.startsWith('gpt-5-nano')) ctx = 64000;
    else if (lower.startsWith('gpt-5-mini')) ctx = 128000;
    else if (lower.startsWith('gpt-5')) ctx = 200000;
    else if (lower.includes('3.5')) ctx = 16000;
    else if (lower.includes('embedding')) ctx = 8192;
  } else if (provider === 'anthropic') {
  ...
  const providerTone: ModelMeta['providerTone'] = provider === 'openai' || provider === 'anthropic' ? 'accent' : 'neutral';
  return { family, ctx, tags, providerTone };
}
