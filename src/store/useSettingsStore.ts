...
import { isUsableOpenAIChatModel, PRIMARY_OPENAI_GPT5_MODEL } from '@/utils/ai/models/openaiGpt5';
...
function normalizeSettingsPayloadModel(data: SettingsPayloadV2): SettingsPayloadV2 {
  const provider = ((data as any)?.settings?.provider || data.provider || 'openai') as string;
  if (provider !== 'openai') {
    return data;
  }

  const currentModel = String((data as any)?.settings?.model || data.model || '').trim();
  if (currentModel && isUsableOpenAIChatModel(currentModel)) {
    return data;
  }

  const next = { ...(data as any), model: PRIMARY_OPENAI_GPT5_MODEL } as SettingsPayloadV2 & { settings?: { model?: string } };
  if (next.settings) {
    next.settings = { ...next.settings, model: PRIMARY_OPENAI_GPT5_MODEL };
  }
  return next;
}

function normalizeLoadedSettingsState(state: SettingsStateV2): SettingsStateV2 {
  let changed = false;
  const profiles = state.profiles.map((profile) => {
    const normalizedData = normalizeSettingsPayloadModel(profile.data);
    if (normalizedData === profile.data) {
      return profile;
    }
    changed = true;
    return { ...profile, data: normalizedData, updatedAt: Date.now() };
  });
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  const loaded = Adapter.load();
    if (loaded) {
      const normalized = normalizeLoadedSettingsState(loaded);
      set({ ...normalized, dirty: false });
    ...
    try {
      ...
      legacy.loadSettings().then((old: unknown) => {
        ...
        const normalized = normalizeLoadedSettingsState(migrated);
        set(normalized);
        Adapter.save(normalized);
      }).catch(() => {
  ...
  async savePersisted() {
    try {
      ...
      const model = (active.data as any)?.settings?.model || (active.data as any)?.model || 'gpt-5-mini';
