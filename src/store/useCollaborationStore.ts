...
interface CollaborationStore extends CollaborationState {
  ...
  startCollaboration: (session: CoAnalysisSession) => void;
...
function pickCollaboratorColor(seed: string): string {
  return COLLABORATOR_COLOR_PALETTE[hashSeed(seed) % COLLABORATOR_COLOR_PALETTE.length];
}

function shouldNormalizeLegacyColor(color: string): boolean {
  const v = color.trim().toLowerCase();
  if (!v) return true;
  if (v.includes('hsl(') || v.includes('rgb(') || v.includes('rgba(')) return true;
  if (/^[a-z-]+$/.test(v)) return true;
  return false;
}

function normalizeCollaboratorColor(
  color: unknown,
  fallbackSeed: string
): string {
  if (typeof color !== 'string') return pickCollaboratorColor(fallbackSeed);
  if (shouldNormalizeLegacyColor(color)) return pickCollaboratorColor(`${fallbackSeed}:${color}`);
  return color;
}
...
export const useCollaborationStore = create<CollaborationStore>((set, get) => ({
  ...
  endCollaboration: () => {
    const session = get().coAnalysisSession;
    if (session) {
      get().addActivity({
        id: crypto.randomUUID(),
        type: 'session_left',
        ...
        targetId: session.id,
        targetType: 'session',
        description: `Ended collaboration session`,
        timestamp: new Date(),
      });
    }
    set({
  ...
  inviteUser: (email, role) => {
    ...
    const newParticipant: CollaborationUser = {
      ...
      email: email.trim(),
      role: (role || 'analyst') as UserRole,
      color: pickCollaboratorColor(email.trim().toLowerCase()),
      isOnline: false,
      lastSeen: new Date(),
    };
  ...
  inviteToSession: (userId, role) => {
    ...
    if (session && user) {
      const newParticipant: CoAnalysisSession['participants'][0] = {
        userId: user.id,
        role,
        joinedAt: new Date(),
        permissions: {
      ...
      get().addActivity({
        ...
        targetId: session.id,
        targetType: 'session',
        description: `Joined collaboration session as ${role}`,
        timestamp: new Date(),
      });
      get().saveToStorage();
    }
  ...
  loadFromStorage: () => {
    try {
      ...
      if (stored) {
        ...
        const normalizeUser = (u: CollaborationUser | null): CollaborationUser | null => {
          if (!u) return null;
          return {
            ...u,
            color: normalizeCollaboratorColor(u.color, u.id || u.email || u.name || 'user'),
          };
        };

        const normalizedParticipants = (parsed.participants || []).map((p: CollaborationUser) => ({
          ...p,
          color: normalizeCollaboratorColor(p.color, p.id || p.email || p.name || 'participant'),
        }));

        set({
          currentUser: normalizeUser(parsed.currentUser || null),
          coAnalysisSession: parsed.coAnalysisSession || null,
  ...
  saveToStorage: () => {
    try {
      ...
      const toStore = {
        ...
        coAnalysisSession: state.coAnalysisSession,
...
export const useCoAnalysisSession = () => useCollaborationStore((s) => s.coAnalysisSession);
