...
export function useClipboard(
  ...
  const log = useCallback((type: ClipboardEventType, msg: string, payload?: ClipboardEventPayload) => {
    console.log(`[Clipboard] ${msg}`, payload || '');
    onEvent?.(type, msg, payload);
  }, [onEvent]);

  const genId = useCallback(() => `file-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`, []);

  const cloneWithNewIds = useCallback((node: FileNode, baseDestPath: string): FileNode => {
  ...
  const uniquifyName = useCallback((name: string, parentPath: string): string => {
  ...
  const rebuildPaths = useCallback((node: FileNode, parentPath: string) => {
