...
3. Run `.\scripts\get-next-urban-analytics-prompt.ps1 -Json` after setting process execution policy when needed.
4. Search for the returned prompt block in `DEVELOPMENT_PLANS/URBAN_ANALYTICS_SEQUENTIAL_IMPLEMENTATION_PROMPTS.md`.
5. Implement only that prompt unless the user explicitly asks for a different prompt.
6. Finish with `DEVELOPMENT_PLANS/URBAN_ANALYTICS_AGENT_HANDOFF_TEMPLATE.md`.
