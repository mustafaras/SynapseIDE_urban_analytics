...
```text
Prompt 08 - Inspector Host (of 56)
```
...
```text
Prompt 07 pushed (implementation commit `35468fe`; integration tip includes ledger closeout)
```
...
| [x] | 07 - Data Consolidation | `gis-modal-ui/p07-data-consolidation` | `35468fe` | pushed | Add Data/Connections/Catalog/Source Health/Demo Data unified in one Data activity |
| [ ] | 08 - Inspector Host | `gis-modal-ui/p08-inspector-host` |  |  | LayerInspector hosted in right inspector |
| [ ] | 09 - QA Problems Panel | `gis-modal-ui/p09-qa-problems` |  |  | Problems surface for QA blockers/warnings |
| [ ] | 10 - Bottom Panel | `gis-modal-ui/p10-bottom-panel` |  |  | Problems/Attributes/Timeline/Tasks/Diagnostics tabs |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-06-01 | 07 - Data Consolidation | `gis-modal-ui/p07-data-consolidation` | `2cae370` | `35468fe` | pushed to `origin/gis-modal-ui/p07-data-consolidation` | fast-forwarded and pushed `origin/gis-modal-ui/premium-redesign` | `npm run typecheck` pass; `npx vitest run src/services/map/__tests__/MapDataIO.test.ts` pass (30/30); `npx vitest run src/services/map/__tests__/MapSourceRegistry.test.ts` pass (3/3); `npx vitest run src/centerpanel/components/map/__tests__/MapCatalogPanel.test.tsx` pass (5/5); `npm run lint:errors` pass | Data activity now owns Add Data, Connections, Catalog, Source Health, and Demo Data tabs in the existing sidebar; the toolbar catalog command routes to `data-catalog` instead of opening a competing floating catalog; `MapCatalogPanel` has an embedded presentation that removes outer header/close chrome while keeping the floating default for direct component use; Add Data triggers the import hub and keeps truthful local format limits for GeoJSON/CSV/Arrow/GeoParquet/KML/KMZ/GPX/Shapefile/GPKG plus sampled/profile-only caveats; Connections can open the external service dialog and keeps provider/CORS/rate-limit/no-credential caveats visible; Source Health surfaces restored/live, recoverable, unavailable, external, metadata-only, and demo/synthetic counts with per-source restore chips; Demo Data keeps the DEMO / SYNTHETIC labels and registered source insertion path | Fresh GIS Modal Premium UI Redesign pack confirmed; archived production GIS ladder not resumed; Prompt 07 branch created from pushed integration start SHA `2cae370`; prompt-specific files read plus data/sidebar/navigation seams; changed files: `MapDataImportHubDialog.tsx`, `MapCatalogPanel.tsx`, `MapCatalogPanel.module.css`, `catalogModel.ts`, `catalog/index.ts`, `MapExplorerModalComposition.tsx`, `mapNavigationModel.ts`, `GisTabs.tsx`, `MapCatalogPanel.test.tsx`; no Urban Analytics, Synapse IDE, raw source-byte persistence, credential persistence, or archived operating-pack edits | External service discovery remains in the existing modal because it owns provider-specific health flows; the Data activity intentionally triggers that flow rather than embedding all service-dialog internals. Readiness counts are source-state metrics, so an unavailable metadata-only source can contribute to both unavailable and metadata-only counts |
