...
if (-not $ManifestPath) {
  $ManifestPath = Join-Path $repoRoot "DEVELOPMENT_PLANS\URBAN_ANALYTICS_PROMPT_MANIFEST.json"
}

if (-not $LedgerPath) {
  $LedgerPath = Join-Path $repoRoot "DEVELOPMENT_PLANS\URBAN_ANALYTICS_IMPLEMENTATION_LEDGER.md"
}
...
Write-Host "Next Urban Analytics prompt:"
