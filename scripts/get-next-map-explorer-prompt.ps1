...
if (-not $ManifestPath) {
  $ManifestPath = Join-Path $repoRoot "DEVELOPMENT_PLANS\MAP_EXPLORER_PROMPT_MANIFEST.json"
}

if (-not $LedgerPath) {
  $LedgerPath = Join-Path $repoRoot "DEVELOPMENT_PLANS\MAP_EXPLORER_IMPLEMENTATION_LEDGER.md"
}
...
if (-not $next) {
  $result = [ordered]@{
    ...
    message = "All Map Explorer prompts are completed or skipped with reason."
    nextPrompt = $null
...
Write-Host "Next Map Explorer prompt:"
