---
name: add-ua-card
description: Add a new analytical method, indicator, or dataset card to the Urban Analytics seed library. Use when asked to add a new UA method, add a card to the analytics library, register a new indicator, add a new seed, or update Urban Analytics cards because a GIS modal bridge prompt explicitly requires a typed analytical capability.
---
...
In the GIS Modal Premium UI pack, do not add or alter Urban Analytics cards for visual polish. Only use this skill when the selected prompt explicitly requires a typed bridge capability or analytical library addition.
...
- Never set `score` directly; that is computed at runtime from actual data.
...
If the change was made for a GIS modal bridge prompt, also run the relevant Map Explorer bridge tests:
